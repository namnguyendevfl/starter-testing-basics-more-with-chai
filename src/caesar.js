...
const caesarModule = (function () {
  ...
  function caesar(input, shift, encode = true) {
    ...
    let alphabet = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z']
        alphabet = alphabet.concat(alphabet) // join 2 standard alphabets
    ...
    if (encode) {
      for (let i = 0; i<input.length; i++){
        ...
        // preserve space or upperCase letters
        inputChar = inputChar.toLowerCase()
        if ((inputChar === " ") || (!alphabet.includes(inputChar))) result += inputChar
        // if there are no space or upperCase letters, start shifting
        else {
    ...
    //Decode the given message
      ...
      for (let i = 0; i<input.length; i++){
        ...
        // if there are no space or upperCase, start shifting
          ...
          const idxOfInputChar = alphabet.indexOf(inputChar)
          // if shift > 0, start decoding from the index of the input in the first alphabet
          if (shift < 0) {
            const newPstn = idxOfInputChar - shift
            const newChar = alphabet[newPstn]
            result += newChar}
            // if shift < 0, start deconding from the index of the input in the second alphabet
          else {
            const negIdxOfInputChar = alphabet.indexOf(inputChar, idxOfInputChar + 1)
            const newPstn = negIdxOfInputChar - shift
            const newChar = alphabet[newPstn]
